...
describe('Domain Checker Business Logic', () => {
  ...
  describe('API Route Handler Validations', () => {
    it('throws error if no base domain provided', async () => {
      vi.mocked(readBody).mockResolvedValueOnce({ tlds: ['.com'] })
      await expect((checkDomainsHandler as (event: unknown) => Promise<{ domains: unknown[] }>)({} as never)).rejects.toHaveProperty('statusCode', 400)
    })

    it('throws error if domain is too long', async () => {
      vi.mocked(readBody).mockResolvedValueOnce({ domain: 'a'.repeat(64), tlds: ['.com'] })
      await expect((checkDomainsHandler as (event: unknown) => Promise<{ domains: unknown[] }>)({} as never)).rejects.toHaveProperty('statusCode', 400)
    })

    it('returns results for valid requests', async () => {
      vi.mocked(readBody).mockResolvedValueOnce({ domain: 'test', tlds: ['.com'] })
      mockFetch.mockResolvedValueOnce({ Status: 3 })
      mockFetch.mockResolvedValueOnce({ Status: 3 })
      const res = await (checkDomainsHandler as (event: unknown) => Promise<{ domains: unknown[] }>)({} as never)
      expect(res.domains.length).toBe(1)
    })

    it('throws error for invalid TLD format', async () => {
      vi.mocked(readBody).mockResolvedValueOnce({ domain: 'test', tlds: 'not-an-array' })
      await expect((checkDomainsHandler as (event: unknown) => Promise<{ domains: unknown[] }>)({} as never)).rejects.toHaveProperty('statusCode', 400)
    })

    it('throws error for non-string TLDs', async () => {
      vi.mocked(readBody).mockResolvedValueOnce({ domain: 'test', tlds: [123] })
      await expect((checkDomainsHandler as (event: unknown) => Promise<{ domains: unknown[] }>)({} as never)).rejects.toHaveProperty('statusCode', 400)
    })
    ...
    it('throws error for too many TLDs', async () => {
      vi.mocked(readBody).mockResolvedValueOnce({ domain: 'test', tlds: Array(101).fill('.com') })
      await expect((checkDomainsHandler as (event: unknown) => Promise<{ domains: unknown[] }>)({} as never)).rejects.toHaveProperty('statusCode', 400)
    })

    it('throws error if base domain + tld is too long', async () => {
      vi.mocked(readBody).mockResolvedValueOnce({ domain: 'a'.repeat(250), tlds: ['.com'] })
      await expect((checkDomainsHandler as (event: unknown) => Promise<{ domains: unknown[] }>)({} as never)).rejects.toHaveProperty('statusCode', 400)
    })
  })
})
